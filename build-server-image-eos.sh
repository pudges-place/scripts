...
#   cp -r configs/ MP2/home/alarm/
    case $PLATFORM in
      RPi64)    cp rpi4-config.txt MP2/home/alarm/ ;;
      OdroidN2) cp n2-boot.ini MP2/home/alarm ;;
    esac
    printf "$PLATFORM\n" > platformname
    printf "$LOCAL\n" > mirrors
    printf "$OPSYS\n" > operatingsystem
...
_create_image(){
    case $PLATFORM in
       OdroidN2)# time bsdtar --use-compress-program=zstdmt -cf /home/$USERNAME/pudges-place/server-images/enosARM-server-odroid-n2-latest.tar.zst *
          time bsdtar -cf - * | zstd -z --rsyncable -10 -T0 -of /home/$USERNAME/pudges-place/server-images/enosARM-server-odroid-n2-latest.tar.zst
          ...
          cd $dir ;;
       RPi64) # time bsdtar --use-compress-program=zstdmt -cf /home/$USERNAME/pudges-place/server-images/enosARM-server-rpi-latest.tar.zst *
...
_help() {
   ...
   printf " -o  enter OS type: d or s (d=dekstop s=server)\n"
   ...
   printf "example: sudo ./build-image-eos -d sda -p rpi -o s -i y -c y -l n\n"
   printf "Ensure that the directory /home/\$username/pudges-place/server-images exists\n\n"
}
